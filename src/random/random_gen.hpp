...
class Rng {
  ...
  public:
    ...
    std::vector<T> vector(size_t n, Dist &dist) {
        std::vector<T> v(n);
        for (auto &x : v)
            x = dist(rng);
        return v;
    ...
    template <typename T>
    T any(std::vector<T> &v) {
        std::uniform_int_distribution<size_t> index_dist(0, v.size() - 1);
        size_t random_index = index_dist(rng);
     ...
     */
    template <typename T, typename Dist>
    std::vector<std::vector<T>> matrix(size_t n, size_t m, Dist &dist) {
